...
export default function LoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error || !user) {
        throw new Error("Invalid credentials"); // More accurate error message
      }

      // Basic Password Validation (Replace with your actual validation)
      //if (!bcrypt.compareSync(password, user.password)) {  // Assuming you're using bcrypt
      //   throw new Error("Invalid credentials");
      //}
      console.log(user.user_id);

      // **Crucial: Set the user cookie**
      document.cookie = `user=${encodeURIComponent(
        JSON.stringify({ id: user.user_id, role: user.role })
  ...
  // (Rest of your LoginPage component - the form, etc.)
  return (
        ...
        <div className="flex-grow">
          ...
          <div className="flex flex-col space-y-4 mt-6">
            <button className="flex items-center justify-center w-full py-3 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 focus:outline-none transition">
              <img
                src="./auth/path_to_facebook_icon.png"
                alt="Facebook"
            ...
            <button className="flex items-center justify-center w-full py-3 bg-red-600 text-white font-semibold rounded-md hover:bg-red-700 focus:outline-none transition">
              <img
                src="./auth/path_to_google_icon.png"
                alt="Google"
                className="w-5 h-5 mr-2"
              />
              Log in with Google
